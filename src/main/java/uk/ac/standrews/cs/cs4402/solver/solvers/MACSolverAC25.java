...
public class MACSolverAC25 implements Solver {
    BinaryCSPGraph bcsp;
    boolean debug = false;

    private DelegateTree<String, String> searchTree = new DelegateTree<>();
    private String searchTreeActiveNode;
    public MACSolverAC25(){
        this(false);
    }
    public MACSolverAC25(boolean debug){
        this.debug=debug;
    }
    @Override
    public void setCSP(BinaryCSPGraph csp) {
        bcsp = csp;
        bcsp.reviseArcs_AC25(); // reduce variable domains if possible
        bcsp.push();
    ...
    public boolean step(VariableOrderingHeuristic varH, ValueOrderingHeuristic valH, boolean slow) {
        ...
        try {//assign left node, i.e. assign values to variable
            ...
            try {
                bcsp.reviseArcs_AC25(currentCSPVar.getId());
            }
            finally {
